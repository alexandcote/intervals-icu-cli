...
export function readFileConfig(): FileConfig {
  ...
  try {
    ...
    throw new CliError(
      ...
      `Fix or remove it, then run: intervals config set api_key`,
    )
  }
}
...
export function requireApiKey(config: ResolvedConfig): string {
  if (!config.apiKey) {
    throw new CliError(
      ...
      'Set INTERVALS_API_KEY or run `intervals config set api_key` and enter the key when prompted. Get a key at https://intervals.icu/settings (Developer Settings).',
    )
  }
